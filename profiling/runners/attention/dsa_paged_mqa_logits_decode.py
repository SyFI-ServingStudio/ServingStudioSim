...
_EXPECTED_SMS = {"NVIDIA H200": 132, "NVIDIA B200": 148}
...
def _validate_cuda_device(torch: Any) -> None:
    ...
    if gpu_name not in _SUPPORTED_GPUS:
        raise ProfilerNotImplemented(
            "torch dsa_paged_mqa_logits_decode is verified only on "
            f"{sorted(_SUPPORTED_GPUS)}, got {gpu_name}"
        )


def _validate_deepgemm_cuda_device(torch: Any) -> int:
    """Validate the profiled H200/B200 deployment identity and SM schedule."""
    if not torch.cuda.is_available():
        raise ProfilerNotImplemented(
    ...
    expected_sms = _EXPECTED_SMS[gpu_name]
