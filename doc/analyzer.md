...
and arbitrary origins, bound response size and request time, and preserve the
HTTP response JSON without recomputing metrics. An agent discovers experiments
through `/sweeps`, discovers coordinates and scalar metric keys through a
sweep payload, follows the returned opaque `run_id` for run-level resources,
and then follows topology/operation identifiers for worker, iteration,
cost-tree, and kernel drill-down. The active `sweep_id` is UI turn context; an
agent must not guess it from catalog ordering outside an explicit smoke test.

The adapter has two explicit ownership modes. In `local` mode (the default for
an agent loop), it starts a loopback-only Analyzer child inside the Codex
container and reads that conversation's `/workspace/logs`. In `external` mode
(a UI examining pre-existing host experiments), it reads a configured Analyzer
origin and requires the UI to supply the selected `sweep_id` as turn context.
Merely setting an external origin must not silently redirect a local agent
loop; source ownership is configuration, not catalog discovery.
