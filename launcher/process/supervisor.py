...
class ProcessSupervisor:
    ...
    async def run(self, spec: ProcessSpec) -> ProcessResult:
        with ExitStack() as resources:
            ...
            stderr_output = self._read_capture(stderr_stream)
    ...
    def _open_stderr(spec: ProcessSpec, resources: ExitStack) -> BinaryIO | None:
        """A second regular file for stderr, or `None` to keep the inherited
        behaviour (folded into stdout, or the parent's stderr)."""
        if not spec.separate_stderr:
            return None
        return resources.enter_context(tempfile.TemporaryFile(prefix="vibesim-process-err-"))
