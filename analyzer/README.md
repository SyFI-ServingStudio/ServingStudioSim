...
| `optimality` | optimality | `cost_log` (exact R0/R1 + 1/stride-sampled fold) + CostTree manifests + `run_meta` `gpu_ids` counts + `gpu/spec.json` roofline + `raw/kernel_grid_peaks.json` sidecar | sub-optimality waterfall in GPU·s — `idle`/`imbalance`/`batching`/`communication`/`hardware_gap`/`hardware_optimal` telescoping buckets that sum to Real, at cluster / pool / worker / iteration / per-kernel levels / `optimality_waterfall` |
