...
use super::ladder::{AggregateLevel, KernelContribution, KernelLadder, KernelRungs};
use super::levels::BaseRungs;
use super::prepare::KernelLocation;
use super::{ms_to_s, TOP_KERNELS};
...
pub(super) fn worker_kernel_ladders(
    kernel_locations: &[KernelLocation],
    workers: &[WorkerFoldAccumulator],
    worker_rungs_in_fold_order: &[BaseRungs],
    sampled_rungs_by_location_worker: &HashMap<(u32, usize), [f64; 4]>,
    worker_anchor_factors: &[f64],
) -> Vec<KernelLadder> {
    ...
    worker_indices
        ...
        .map(|worker_index| {
            ...
            let kernels: Vec<KernelContribution> = kernel_rows
                .into_iter()
                .map(|(location_index, kernel_rungs)| {
                    ...
                    KernelContribution {
                        ...
                        rungs: KernelRungs::from_gpu_ms(kernel_rungs),
                        necessary_work: None,
                    }
                })
                .collect();
            KernelLadder::worker(&worker.pool_tag, worker.worker_id, worker_rungs, kernels)
        })
        .collect()
}

/// Reduce analyzer-owned worker ladders into explicit pool and cluster ladders.
/// The typed domain adds R0..R5 directly but recomputes R6/R7 from additive work.
pub(super) fn aggregate_kernel_ladders(
    worker_ladders: &[KernelLadder],
) -> Result<Vec<KernelLadder>> {
    if worker_ladders.is_empty() {
        return Ok(Vec::new());
    }
    let all_members: Vec<&KernelLadder> = worker_ladders.iter().collect();
    let mut pools: BTreeMap<String, Vec<&KernelLadder>> = BTreeMap::new();
    for ladder in worker_ladders {
        let (pool_tag, _worker_id) = ladder
            .worker_ref()
            .context("worker ladder collection contains an aggregate scope")?;
        pools.entry(pool_tag.to_string()).or_default().push(ladder);
    }

    let mut aggregates = vec![KernelLadder::aggregate(
        AggregateLevel::Cluster,
        "cluster",
        "Cluster aggregate",
        &all_members,
    )?];
    for (pool_tag, members) in pools {
        aggregates.push(KernelLadder::aggregate(
            AggregateLevel::Pool,
            &pool_tag,
