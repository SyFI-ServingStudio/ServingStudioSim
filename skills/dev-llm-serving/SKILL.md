...
5. For a broad orientation or a from-scratch build, read
   [`OVERVIEW.md`](OVERVIEW.md).

When a parent workflow supplies a frozen trial brief, treat it as the source of
truth. Use this library to determine **how** to implement that trial, not to pick
...
The copied library intentionally does not vendor full vLLM, SGLang, or
TensorRT-LLM source trees. Engine-source-map references cite paths like
`$SERVE_REPOS/<engine>/...`; point `SERVE_REPOS` at an existing checkout. When
the exact implementation must be rediscovered or verified upstream, route that
read-only investigation through `dev-explore-kernel`.
