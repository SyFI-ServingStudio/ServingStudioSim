...
2. **Pull the file list + churn:** `git diff --stat <base>` (or `<base>...HEAD`).
   ...
   When the same feature crosses `ServingStudioSim/` and a topic worktree, verify the
   ...
   equivalent evidence.
3. **Bucket every changed file by module/layer.** For ServingStudio Sim map each path to:
