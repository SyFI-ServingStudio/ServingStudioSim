...
| `pdl_wait_under_collective_ms` | Every rank's non-synchronizing launch time that started inside a collective on the **same stream** and before that collective ended: PDL `griddepcontrol.wait` residency behind the all-reduce. Each such launch's start is trimmed to the collective's end (a fully covered launch is dropped) **before** the segment races, so the wait is never busy and cannot make an early rank win a segment. Off the path and not additive, like `collective_skew_ms`. Cross-stream overlap is not trimmed. `hidden_under_collective_*` and the per-kernel `overlap` diagnostic still use the untrimmed launches. |
