...
//! identical parameter counts, and the capture in
//! `logs/20260920_0_glm53_dflash2_phase0` shows a `forward` phase whose kernels
//! are the GLM-5.2 ones -- NVFP4 MoE `bmm_E2m1_*`, `rmsNormLamport` +
...
impl Dflash2DraftStage {
    fn build(
        ...
        let attn_allreduce = build_allreduce(&name, "layer.attn_allreduce", resolved, bridge)?;
        let ffn_allreduce = build_allreduce(&name, "layer.ffn_allreduce", resolved, bridge)?;
        Ok(Self {
            name,
            num_draft_layers: resolved.num_draft_layers,
    ...
    fn eval(&self, batch: &NormalizedBatch, draft_tokens: u32, ev: &mut Evaluator) {
        ...
        eval_allreduce(&self.attn_allreduce, query_tokens, self.allreduce_bytes_per_token, ev);
        self.draft_ffn
            .eval(&Dflash2DraftFfnLocalWorkletInput { query_tokens }, ev);
        eval_allreduce(&self.ffn_allreduce, query_tokens, self.allreduce_bytes_per_token, ev);
...
fn build_allreduce(
    ...
    resolved: &Dflash2DraftResolved,
    bridge: &PerfApiBridge,
) -> Result<Op<AllReduceKernel>, BuildError> {
...
fn eval_allreduce(
    op: &Op<AllReduceKernel>,
    tokens: u32,
    bytes_per_token: u32,
    ev: &mut Evaluator,
) {
    let input = AllReduceKernelInput {
        message_size_bytes: u64::from(tokens).saturating_mul(u64::from(bytes_per_token)),
    };
    let metrics = if tokens == 0 {
        LeafMetrics::ZERO
    } else {
