...
use crate::worklet::{
    ...
    Fp8AttnBlockTpWorkletResolved, NativeFp8MoeRouterLocalWorklet,
    NativeFp8MoeRouterLocalWorkletConfig, NativeFp8MoeRouterLocalWorkletInput,
    NativeFp8MoeRouterLocalWorkletResolved, NativeMoeExpertComputeLocalWorklet,
...
pub struct Qwen3MoeFp8DpAttnEpFfnConfigs {
    pub attn_block: Fp8AttnBlockTpWorkletConfig,
    pub moe_router: NativeFp8MoeRouterLocalWorkletConfig,
...
pub struct Qwen3MoeFp8DpAttnEpFfnModel {
    pub name: String,
    pub num_layers: u32,
    pub attn_tp_size: u16,
    pub ep_size: u16,
    ...
    pub attn_block: Fp8AttnBlockTpWorklet,
    pub moe_router: NativeFp8MoeRouterLocalWorklet,
...
pub fn build_configs(
...
) -> Qwen3MoeFp8DpAttnEpFfnConfigs {
    assert!(model.fp8, "native FP8 Qwen L4 requires fp8=true");
    ...
    Qwen3MoeFp8DpAttnEpFfnConfigs {
        ...
        moe_router: NativeFp8MoeRouterLocalWorkletConfig {
            hidden: model.hidden.clone(),
            num_experts: model.num_experts.clone(),
            activation_dtype: model.dtype,
            gpu_name: gpu.clone(),
...
fn total_kv_bytes_per_token(resolved: &Qwen3MoeFp8DpAttnEpFfnResolved) -> Dim {
...
pub fn resolve_configs(cfgs: &Qwen3MoeFp8DpAttnEpFfnConfigs) -> Qwen3MoeFp8DpAttnEpFfnResolved {
    Qwen3MoeFp8DpAttnEpFfnResolved {
        attn_block: Fp8AttnBlockTpWorklet::resolve_config(&cfgs.attn_block),
        moe_router: NativeFp8MoeRouterLocalWorklet::resolve_config(&cfgs.moe_router),
...
pub fn build(
    model_name: String,
    resolved: Qwen3MoeFp8DpAttnEpFfnResolved,
    bridge: &PerfApiBridge,
) -> Result<Qwen3MoeFp8DpAttnEpFfnModel, BuildError> {
    ...
    let attn_block = Fp8AttnBlockTpWorklet::build(
        format!("{model_name}.attn_block"),
        resolved.attn_block,
        bridge,
    )?;

    let moe_router = NativeFp8MoeRouterLocalWorklet::build(
    ...
    let mut model = Qwen3MoeFp8DpAttnEpFfnModel {
        name: model_name,
        num_layers,
        attn_tp_size,
        ep_size,
    ...
    fn eval_into(&self, batch: &UnifiedArchInput, ev: &mut Evaluator) {
        ...
        for g in &batch.groups {
            self.moe_router.eval(
                &NativeFp8MoeRouterLocalWorkletInput {
                    batch_tokens: g.batch_tokens,
                },
                ev,
            );
...
mod tests {
    ...
    #[test]
    fn native_fp8_graph_has_p2p_combine_only() {
