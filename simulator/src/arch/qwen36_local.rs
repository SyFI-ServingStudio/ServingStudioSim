...
use crate::worklet::{
    ...
    Qwen36GdnLocalWorkletResolved, Qwen36HeadLocalWorklet, Qwen36HeadLocalWorkletConfig,
    Qwen36HeadLocalWorkletInput, Qwen36HeadLocalWorkletResolved,
...
const ROUTED_GEMM_BACKENDS: &[&str] = &["flashinfer_trtllm"];
...
fn parse_model_json(
    ...
    ensure!(raw.model_type == "qwen3_5_moe", "outer model_type must be qwen3_5_moe");
    ensure!(!raw.tie_word_embeddings, "outer embeddings must be untied");
    ensure!(raw.vision_config.is_object(), "pinned checkpoint must contain vision_config");

    let t = raw.text_config;
    ensure!(t.model_type == "qwen3_5_moe_text", "text model_type must be qwen3_5_moe_text");
    ...
    for (name, actual, expected) in [
        ...
        ("linear_num_key_heads", t.linear_num_key_heads, GDN_KEY_HEADS),
        ("linear_num_value_heads", t.linear_num_value_heads, GDN_VALUE_HEADS),
        ("linear_key_head_dim", t.linear_key_head_dim, GDN_KEY_DIM),
        ("linear_value_head_dim", t.linear_value_head_dim, GDN_VALUE_DIM),
        ("linear_conv_kernel_dim", t.linear_conv_kernel_dim, GDN_CONV_KERNEL),
        ("num_experts", t.num_experts, NUM_EXPERTS),
        ("num_experts_per_tok", t.num_experts_per_tok, TOP_K),
        ("moe_intermediate_size", t.moe_intermediate_size, EXPERT_WIDTH),
        ("shared_expert_intermediate_size", t.shared_expert_intermediate_size, EXPERT_WIDTH),
        ("mtp_num_hidden_layers", t.mtp_num_hidden_layers, 1),
    ] {
        ensure!(actual == expected, "{name} must be {expected}, got {actual}");
    }
    ensure!(t.partial_rotary_factor == 0.25, "partial_rotary_factor must be 0.25");
    ensure!(t.attn_output_gate, "attention output gate must be enabled");
    ensure!(t.mamba_ssm_dtype == "float32", "GDN recurrent state must be float32");
    ...
    ensure!(q.activation_scheme == "dynamic", "FP8 activation scheme must be dynamic");
    ensure!(q.fmt == "e4m3", "FP8 format must be e4m3");
    ensure!(q.quant_method == "fp8", "quant_method must be fp8");
    ensure!(q.weight_block_size == [FP8_GROUP, FP8_GROUP], "weight block must be 128x128");
    for required in ["lm_head", "model.embed_tokens"] {
        ensure!(q.modules_to_not_convert.iter().any(|entry| entry == required), "missing FP8 exclusion {required}");
    }
    for layer in 0..CHECKPOINT_LAYERS {
        let shared_gate = format!("model.language_model.layers.{layer}.mlp.shared_expert_gate");
        ensure!(q.modules_to_not_convert.iter().any(|entry| entry == &shared_gate), "missing shared gate FP8 exclusion for layer {layer}");
        if layer % 4 != 3 {
            let ba = format!("model.language_model.layers.{layer}.linear_attn.in_proj_ba");
            ensure!(q.modules_to_not_convert.iter().any(|entry| entry == &ba), "missing BA FP8 exclusion for layer {layer}");
        }
    }
    ...
    ensure!((1..=CHECKPOINT_LAYERS).contains(&logical_num_layers), "num_layers must be in 1..=40");
    ensure!((1..=logical_num_layers).contains(&num_layers), "sim_num_layers must be in 1..=num_layers");
...
pub fn build_configs(model: &Qwen36ModelCfg, parallel: &Qwen36LocalParallel) -> Qwen36LocalConfigs {
    ...
    Qwen36LocalConfigs {
        embedding: ElementwiseKernelConfig {
            ...
            input_bytes_per_token: EMBED_INPUT_BYTES.into(), output_bytes_per_token: EMBED_OUTPUT_BYTES.into(),
        },
        gdn: Qwen36GdnLocalWorkletConfig {
            hidden: model.hidden.clone(), num_key_heads: model.linear_num_key_heads.clone(),
            num_value_heads: model.linear_num_value_heads.clone(), key_head_dim: model.linear_key_head_dim.clone(),
            value_head_dim: model.linear_value_head_dim.clone(), conv_kernel_size: model.linear_conv_kernel_dim.clone(),
            activation_dtype: model.activation_dtype, conv_state_dtype: model.conv_state_dtype,
            ssm_state_dtype: model.ssm_state_dtype, gpu_name: gpu.clone(),
            residual_rms_norm_backends: RESIDUAL_NORM_BACKENDS.to_vec(), fp8_quant_backends: DENSE_FP8_QUANT_BACKENDS.to_vec(),
            fp8_gemm_backends: DENSE_FP8_GEMM_BACKENDS.to_vec(), bf16_gemm_backends: BF16_GEMM_BACKENDS.to_vec(),
            elementwise_backends: ELEMENTWISE_BACKENDS.to_vec(), causal_conv_prefill_backends: GDN_BACKENDS.to_vec(),
            prefill_post_conv_backends: GDN_BACKENDS.to_vec(), chunk_cumsum_backends: GDN_BACKENDS.to_vec(),
            chunk_kkt_backends: GDN_BACKENDS.to_vec(), chunk_solve_backends: GDN_BACKENDS.to_vec(),
            recompute_w_u_backends: GDN_BACKENDS.to_vec(), chunk_state_update_backends: GDN_BACKENDS.to_vec(),
            chunk_output_backends: GDN_BACKENDS.to_vec(), causal_conv_decode_backends: GDN_BACKENDS.to_vec(),
            recurrent_decode_backends: GDN_BACKENDS.to_vec(), gated_rms_norm_backends: GDN_BACKENDS.to_vec(),
        },
        gated_gqa: Qwen36GatedGqaLocalWorkletConfig {
            hidden: model.hidden.clone(), num_qo_heads: model.num_qo_heads.clone(), num_kv_heads: model.num_kv_heads.clone(),
            head_dim: model.head_dim.clone(), rope_dim: model.rope_dim.clone(), activation_dtype: model.activation_dtype,
            gpu_name: gpu.clone(), kv_cache_block_size: 16, kv_cache_layout: "NHD".into(),
            kv_scale_granularity: "tensor".into(), residual_rms_norm_backends: RESIDUAL_NORM_BACKENDS.to_vec(),
            fp8_quant_backends: DENSE_FP8_QUANT_BACKENDS.to_vec(), fp8_gemm_backends: DENSE_FP8_GEMM_BACKENDS.to_vec(),
            qk_rms_norm_backends: QK_NORM_BACKENDS.to_vec(), elementwise_backends: ELEMENTWISE_BACKENDS.to_vec(),
            attention_backends: GQA_ATTN_BACKENDS.to_vec(), kv_cache_append_backends: KV_APPEND_BACKENDS.to_vec(),
        },
        router: Qwen36MoeRouterLocalWorkletConfig {
            hidden: model.hidden.clone(), num_experts: model.num_experts.clone(), top_k: model.top_k,
            block_size: model.alignment_block_size, activation_dtype: model.activation_dtype, gpu_name: gpu.clone(),
            bf16_gemm_backends: BF16_GEMM_BACKENDS.to_vec(),
            fused_topk_backends: ROUTING_BACKENDS.to_vec(), align_backends: ROUTING_BACKENDS.to_vec(),
        },
        routed_expert: VllmFp8MoeExpertComputeLocalWorkletConfig {
            hidden: model.hidden.clone(), moe_intermediate: model.moe_intermediate.clone(), num_experts: model.num_experts.clone(),
            ep_size: 1, top_k: model.top_k, activation_dtype: model.activation_dtype, gpu_name: gpu.clone(),
            act_backends: ELEMENTWISE_BACKENDS.to_vec(), fp8_quant_backends: ROUTED_QUANT_BACKENDS.to_vec(),
            fp8_grouped_gemm_backends: ROUTED_GEMM_BACKENDS.to_vec(), local_ppm: ppm.clone(),
        },
        shared_expert: Qwen36SharedExpertLocalWorkletConfig {
            hidden: model.hidden.clone(), intermediate: model.shared_intermediate.clone(),
            num_shared_experts: model.num_shared_experts, activation_dtype: model.activation_dtype, gpu_name: gpu.clone(),
            fp8_quant_backends: DENSE_FP8_QUANT_BACKENDS.to_vec(), fp8_gemm_backends: DENSE_FP8_GEMM_BACKENDS.to_vec(),
            bf16_gemm_backends: BF16_GEMM_BACKENDS.to_vec(), elementwise_backends: ELEMENTWISE_BACKENDS.to_vec(),
        },
        finalize: Qwen36MoeFinalizeLocalWorkletConfig {
            hidden: model.hidden.clone(), num_experts: model.num_experts.clone(), top_k: model.top_k,
            activation_dtype: model.activation_dtype, local_ppm: ppm, gpu_name: gpu.clone(),
            finalize_backends: FINALIZE_BACKENDS.to_vec(), elementwise_backends: ELEMENTWISE_BACKENDS.to_vec(),
        },
        head: Qwen36HeadLocalWorkletConfig {
            hidden: model.hidden.clone(), vocab_size: model.vocab_size.clone(), activation_dtype: model.activation_dtype,
            gpu_name: gpu, residual_rms_norm_backends: RESIDUAL_NORM_BACKENDS.to_vec(),
            bf16_gemm_backends: BF16_GEMM_BACKENDS.to_vec(),
        },
        logical_num_layers: model.logical_num_layers, num_layers: model.num_layers,
        num_gdn_layers: model.num_gdn_layers, num_gqa_layers: model.num_gqa_layers,
    }
}
...
pub fn resolve_configs(cfg: &Qwen36LocalConfigs) -> Qwen36LocalResolved {
    Qwen36LocalResolved {
        ...
        logical_num_layers: cfg.logical_num_layers, num_layers: cfg.num_layers,
        num_gdn_layers: cfg.num_gdn_layers, num_gqa_layers: cfg.num_gqa_layers,
    }
}
...
pub fn build(name: String, resolved: Qwen36LocalResolved, bridge: &PerfApiBridge) -> std::result::Result<Qwen36LocalModel, BuildError> {
    ...
    let mut model = Qwen36LocalModel {
        ...
        gated_gqa: Qwen36GatedGqaLocalWorklet::build(format!("{name}.gated_gqa"), resolved.gated_gqa, bridge)?,
        router: Qwen36MoeRouterLocalWorklet::build(format!("{name}.router"), resolved.router, bridge)?,
        routed_expert: VllmFp8MoeExpertComputeLocalWorklet::build(format!("{name}.routed_expert"), resolved.routed_expert, bridge)?,
        shared_expert: Qwen36SharedExpertLocalWorklet::build(format!("{name}.shared_expert"), resolved.shared_expert, bridge)?,
        finalize: Qwen36MoeFinalizeLocalWorklet::build(format!("{name}.finalize"), resolved.finalize, bridge)?,
...
fn normalize_input(input: &UnifiedArchInput) -> std::result::Result<NormalizedInput, String> {
    if input.groups.len() != 1 { return Err(format!("qwen36_local requires exactly one local group, got {}", input.groups.len())); }
    if !input.tokens_per_source_rank.is_empty() { return Err("qwen36_local requires empty tokens_per_source_rank".into()); }
    ...
    for (index, &(prefix, append)) in group.prefill_chunk_pairs.iter().enumerate() {
        if append == 0 { return Err(format!("prefill request {index} append must be positive")); }
        ...
        let _prefill_total = prefix.checked_add(append).ok_or_else(|| format!("prefill request {index} prefix+append overflow"))?;
        prefill_tokens = prefill_tokens.checked_add(append).ok_or_else(|| "prefill token sum overflow".to_string())?;
        lengths.push(append); has_state.push(prefix > 0);
    }
    if group.prefill_tokens != prefill_tokens { return Err(format!("prefill_tokens {} must equal append sum {prefill_tokens}", group.prefill_tokens)); }
    let decode_tokens = u32::try_from(group.decode_kv_lens.len()).map_err(|_| "decode request count exceeds u32".to_string())?;
    if group.decode_tokens != decode_tokens { return Err(format!("decode_tokens {} must equal decode request count {decode_tokens}", group.decode_tokens)); }
    let mut expected_decode_kv_len = 0_u32;
    for (index, &length) in group.decode_kv_lens.iter().enumerate() {
        if length == 0 { return Err(format!("decode KV length {index} must be positive")); }
        expected_decode_kv_len = expected_decode_kv_len.checked_add(length).ok_or_else(|| "decode KV length sum overflow".to_string())?;
    }
    let active_tokens = prefill_tokens.checked_add(decode_tokens).ok_or_else(|| "active token sum overflow".to_string())?;
    if active_tokens == 0 { return Err("qwen36_local requires nonempty active work".into()); }
    if group.batch_tokens != active_tokens { return Err(format!("batch_tokens {} must equal prefill+decode {active_tokens}", group.batch_tokens)); }
    if group.total_kv_len != expected_decode_kv_len { return Err(format!("total_kv_len {} must equal decode KV sum {expected_decode_kv_len}", group.total_kv_len)); }
    let request_count = u32::try_from(group.prefill_chunk_pairs.len() + group.decode_kv_lens.len()).map_err(|_| "request count exceeds u32".to_string())?;
    let global_expert_selections = active_tokens.checked_mul(TOP_K).ok_or_else(|| "routed selection count overflow".to_string())?;
    Ok(NormalizedInput {
        active_tokens, request_count, global_expert_selections,
        gdn: Qwen36GdnLocalWorkletInput { prefill_sequence_lengths: lengths, prefill_has_initial_state: has_state, decode_batch_size: decode_tokens },
        gated_gqa: Qwen36GatedGqaLocalWorkletInput { batch_tokens: active_tokens, prefill_chunk_pairs: group.prefill_chunk_pairs.clone(), decode_kv_lens: group.decode_kv_lens.clone() },
    })
}

impl Qwen36LocalModel {
    fn layer_node(&self, attention: CostNode, builder: &mut CostTreeBuilder, label: &str) -> CostNode {
        CostNode::Labeled {
            label: label.into(),
    ...
    pub fn cost_tree(&self) -> CostTree {
        ...
        let embedding = CostNode::Labeled { label: "embedding".into(), child: Box::new(self.embedding.compile(&mut builder)) };
        ...
        let decoder = if q == 0 {
            CostNode::Labeled { label: format!("decoder prefix: {r} GDN layer(s)"), child: Box::new(CostNode::Scale { n: r, child: Box::new(gdn_layer) }) }
        } else {
            let gqa_layer = self.layer_node(self.gated_gqa.compile(&mut builder), &mut builder, "gated-GQA layer");
            let cycle = CostNode::Sum(vec![CostNode::Scale { n: 3, child: Box::new(gdn_layer.clone()) }, gqa_layer]);
            let mut prefix = vec![CostNode::Labeled {
                label: format!("decoder cycles: {q} x [3 GDN + 1 gated-GQA]"),
                child: Box::new(CostNode::Scale { n: q, child: Box::new(cycle) }),
            }];
            if r > 0 {
                ...
                prefix.push(CostNode::Labeled {
                    ...
                    child: Box::new(CostNode::Scale { n: r, child: Box::new(gdn_layer) }),
                });
            }
            CostNode::Sum(prefix)
        };
        let head = CostNode::Labeled { label: "head".into(), child: Box::new(self.head.compile(&mut builder)) };
        builder.finish(CostNode::Labeled {
            label: format!("{} (Qwen36LocalModel) [TP1/EP1; {}-layer prefix]", self.name, self.num_layers),
    ...
    fn eval_layer(&self, is_gdn: bool, input: &NormalizedInput, ev: &mut Evaluator) {
        if is_gdn { self.gdn.eval(&input.gdn, ev); } else { self.gated_gqa.eval(&input.gated_gqa, ev); }
        self.router.eval(&Qwen36MoeRouterLocalWorkletInput { batch_tokens: input.active_tokens }, ev);
        self.routed_expert.eval(&VllmFp8MoeExpertComputeLocalWorkletInput { global_expert_selections: input.global_expert_selections }, ev);
        self.shared_expert.eval(&Qwen36SharedExpertLocalWorkletInput { batch_tokens: input.active_tokens }, ev);
        self.finalize.eval(&Qwen36MoeFinalizeLocalWorkletInput { batch_tokens: input.active_tokens }, ev);
    }

    fn eval_into(&self, batch: &UnifiedArchInput, ev: &mut Evaluator) {
        let input = normalize_input(batch).unwrap_or_else(|reason| panic!("invalid Qwen36LocalModel input: {reason}"));
        self.embedding.eval(&ElementwiseKernelInput { num_tokens: input.active_tokens }, ev);
        self.eval_layer(true, &input, ev);
        if self.num_layers >= 4 { self.eval_layer(false, &input, ev); }
        self.head.eval(&Qwen36HeadLocalWorkletInput { final_norm_tokens: input.active_tokens, logits_tokens: input.request_count }, ev);
    }
}

impl IterwiseUnifiedModel for Qwen36LocalModel {
    fn eval_iter(&self, batch: &UnifiedArchInput, slots: &mut Vec<LeafMetrics>, scratch: &mut Vec<LeafMetrics>) -> LeafMetrics {
        slots.clear(); slots.resize(self.n_slots, LeafMetrics::ZERO);
        let mut ev = Evaluator::new(slots); self.eval_into(batch, &mut ev);
        debug_assert_eq!(ev.filled(), self.n_slots, "eval cursor must fill every slot");
        CostTree::aggregate(&self.cost_flat, slots, scratch)
    }

    fn eval_iter_with_inputs(&self, batch: &UnifiedArchInput, slots: &mut Vec<LeafMetrics>, scratch: &mut Vec<LeafMetrics>, inputs: &mut Vec<SlotInput>) -> LeafMetrics {
        slots.clear(); slots.resize(self.n_slots, LeafMetrics::ZERO);
        let mut ev = Evaluator::with_inputs(slots, inputs); self.eval_into(batch, &mut ev);
        debug_assert_eq!(ev.filled(), self.n_slots, "eval cursor must fill every slot");
        let total = CostTree::aggregate(&self.cost_flat, slots, scratch);
        debug_assert_eq!(inputs.len(), self.n_slots, "slot inputs must align with slots"); total
    }
...
mod tests {
    ...
    fn fixture() -> String {
        let layers: Vec<&str> = (0..40).map(|i| if i % 4 == 3 { "full_attention" } else { "linear_attention" }).collect();
        let mut excluded = vec!["lm_head".to_string(), "model.embed_tokens".to_string()];
        for i in 0..40 {
            excluded.push(format!("model.language_model.layers.{i}.mlp.shared_expert_gate"));
            if i % 4 != 3 { excluded.push(format!("model.language_model.layers.{i}.linear_attn.in_proj_ba")); }
        }
        serde_json::json!({
    ...
    fn parses_pinned_nested_identity_and_layer_controls() {
        ...
        assert_eq!((m.hidden.get(), m.vocab_size.get(), m.num_layers, m.num_gdn_layers, m.num_gqa_layers), (2048,248320,40,30,10));
        for (logical, sim, expected) in [(Some(1),None,(1,1,0)),(Some(3),None,(3,3,0)),(Some(4),None,(4,3,1)),(Some(5),None,(5,4,1)),(Some(40),Some(39),(39,30,9))] {
            let m = parse_model_json(&fixture(), logical, sim).unwrap(); assert_eq!((m.num_layers,m.num_gdn_layers,m.num_gqa_layers),expected);
        }
    ...
    fn rejects_identity_schedule_and_quantization_drift() {
        for mutate in [
            ...
            |v: &mut serde_json::Value| v["text_config"]["layer_types"][3] = "linear_attention".into(),
            |v: &mut serde_json::Value| v["quantization_config"]["weight_block_size"] = serde_json::json!([64,128]),
    ...
    fn configs_freeze_backends_shapes_ppm_and_embedding() {
        ...
        assert_eq!(c.finalize.local_ppm.iter().map(|&v|u64::from(v)).sum::<u64>(),1_000_000);
        let r=resolve_configs(&c); assert_eq!((r.gdn.qkvz.gemm.k.get(),r.gdn.qkvz.gemm.n.get()),(2048,12288));
        assert_eq!((r.gated_gqa.qkv_gate.gemm.k.get(),r.gated_gqa.qkv_gate.gemm.n.get()),(2048,9216));
        assert_eq!((r.router.router.k.get(),r.router.router.n.get(),r.router.router.dtype),(2048,256,DType::Bf16));
        assert_eq!(r.router.router.backends,["torch_linear"]); assert_eq!(r.router.router.gpu_name,"NVIDIA H200");
        assert_eq!((r.head.lm_head.k.get(),r.head.lm_head.n.get(),r.head.lm_head.dtype),(2048,248320,DType::Bf16));
        assert_eq!(total_kv_bytes_per_token(&r).get(),20_480);
    }

    #[test]
    fn cost_tree_has_exact_slot_counts_scales_reuse_and_no_communication() {
        for (layers,slots) in [(1,45),(3,45),(4,75),(5,75),(39,75),(40,75)] {
            let m=enumerate_model(layers); let tree=m.cost_tree(); assert_eq!(tree.n_slots(),slots,"L={layers}");
            let kinds:Vec<&str>=tree.slots.iter().map(|s|s.kind.as_str()).collect();
            assert!(!kinds.iter().any(|k| matches!(*k,"all_reduce"|"all_to_all"|"p2p_intra"|"p2p_inter"|"send_recv")));
            assert!(!tree.flatten().iter().any(|n| matches!(n,FlatCostNode::Max{..})));
            if layers==5 {
                let gdn_slots=tree.slots.iter().filter(|s|s.name.starts_with("model.gdn.")).count(); assert_eq!(gdn_slots,25);
                assert!(tree.flatten().iter().any(|n| matches!(n,FlatCostNode::Scale{n,..} if *n==1)));
                let flat = tree.flatten();
                let leaf_ids: Vec<usize> = flat.iter().filter_map(|node| match node { FlatCostNode::Leaf(slot) => Some(*slot), _ => None }).collect();
                assert!(leaf_ids.len() > tree.n_slots(), "remainder must reference existing leaves again");
                let mut unique = leaf_ids.clone(); unique.sort_unstable(); unique.dedup();
                assert_eq!(unique.len(), tree.n_slots(), "remainder must not mint a second GDN slot set");
            }
        }
        let tree=enumerate_model(40).cost_tree();
        const ROUTER_LEAVES: usize = 3;
        const GDN_LAYER_LEAVES: usize = 25 + ROUTER_LEAVES + 5 + 7 + 2;
        ...
        assert_eq!((GDN_LAYER_LEAVES,GQA_LAYER_LEAVES,CYCLE_LEAVES,MODEL_LEAVES),(42,30,72,75));
        assert_eq!(tree.slots.iter().filter(|s|s.name.starts_with("model.gdn.")).count(),25);
        assert_eq!(tree.slots.iter().filter(|s|s.name.starts_with("model.gated_gqa.")).count(),13);
        assert_eq!(tree.slots.iter().filter(|s|s.name.starts_with("model.router.")).count(),6);
        assert_eq!(tree.slots.iter().filter(|s|s.name.starts_with("model.routed_expert.")).count(),10);
        assert_eq!(tree.slots.iter().filter(|s|s.name.starts_with("model.shared_expert.")).count(),14);
        assert_eq!(tree.slots.iter().filter(|s|s.name.starts_with("model.finalize.")).count(),4);
        assert!(!tree.slots.iter().any(|s|s.name=="model.router.router.input_quant"));
        assert_eq!(tree.slots.iter().filter(|s|s.name=="model.router.router.gemm").count(),2);
        let first_routed=tree.slots.iter().position(|s|s.name.starts_with("model.routed_expert.")).unwrap();
        let first_shared=tree.slots.iter().position(|s|s.name.starts_with("model.shared_expert.")).unwrap();
        assert!(first_routed<first_shared,"routed expert must compile before shared expert");
        let manifest=tree.manifest();
        let section_nodes=manifest.node_labels.iter().enumerate().filter_map(|(index,label)| {
            (label.as_deref()==Some("local routed/shared expert resource-conserving section")).then_some(index)
        }).collect::<Vec<_>>();
        assert_eq!(section_nodes.len(),2,"one section per compiled attention archetype");
        assert!(section_nodes.iter().all(|&index| matches!(manifest.nodes[index],FlatCostNode::Sum{..})));
        let unique_names=tree.slots.iter().map(|slot|slot.name.as_str()).collect::<std::collections::BTreeSet<_>>();
        assert_eq!(unique_names.len(),58);
    }

    fn unified(prefill: Vec<(u32,u32)>, decode: Vec<u32>) -> UnifiedArchInput {
        let prefill_tokens=prefill.iter().map(|x|x.1).sum(); let decode_tokens=decode.len() as u32;
        ...
        UnifiedArchInput { groups:vec![ArchGroupInput { batch_tokens:prefill_tokens+decode_tokens,prefill_tokens,decode_tokens,prefill_chunk_pairs:prefill,decode_kv_lens:decode,total_kv_len }],tokens_per_source_rank:vec![] }
    }
    ...
    fn normalization_maps_prefill_decode_mixed_and_chunked_inputs() {
        ...
        assert_eq!((p.active_tokens,p.request_count,p.global_expert_selections),(70,3,560)); assert_eq!(p.gdn.prefill_sequence_lengths,[3,65,2]); assert_eq!(p.gdn.prefill_has_initial_state,[false,false,false]);
        let decode_input=unified(vec![],vec![9,10,11,12]);
        assert_eq!(decode_input.groups[0].total_kv_len,42);
        let d=normalize_input(&decode_input).unwrap(); assert_eq!((d.active_tokens,d.request_count,d.gdn.decode_batch_size),(4,4,4));
        ...
        assert_eq!((m.active_tokens,m.request_count,m.gdn.decode_batch_size),(70,4,2)); assert_eq!(m.gdn.prefill_has_initial_state,[true,false]);
    ...
    fn normalization_rejects_bad_group_accounting_and_overflow() {
        ...
        let mut x=unified(vec![(0,1)],vec![]); x.tokens_per_source_rank=vec![1]; assert!(normalize_input(&x).is_err());
        let mut x=unified(vec![(0,1)],vec![]); x.groups[0].batch_tokens=2; assert!(normalize_input(&x).is_err());
        let mut x=unified(vec![],vec![7,9]); x.groups[0].total_kv_len=15;
        assert!(normalize_input(&x).unwrap_err().contains("decode KV sum 16"));
        assert!(normalize_input(&unified(vec![(0,0)],vec![])).is_err());
        let x=UnifiedArchInput { groups:vec![ArchGroupInput { batch_tokens:1,prefill_tokens:1,decode_tokens:0,prefill_chunk_pairs:vec![(u32::MAX,1)],decode_kv_lens:vec![],total_kv_len:0 }],tokens_per_source_rank:vec![] };
        assert!(normalize_input(&x).unwrap_err().contains("prefix+append overflow"));
        let x=UnifiedArchInput { groups:vec![ArchGroupInput { batch_tokens:2,prefill_tokens:0,decode_tokens:2,prefill_chunk_pairs:vec![],decode_kv_lens:vec![u32::MAX,1],total_kv_len:0 }],tokens_per_source_rank:vec![] };
        assert!(normalize_input(&x).unwrap_err().contains("decode KV length sum overflow"));
        let x=UnifiedArchInput { groups:vec![ArchGroupInput { batch_tokens:u32::MAX,prefill_tokens:u32::MAX,decode_tokens:1,prefill_chunk_pairs:vec![(0,u32::MAX)],decode_kv_lens:vec![1],total_kv_len:0 }],tokens_per_source_rank:vec![] };
    ...
    fn full_manifest_has_one_entry_per_unique_slot() {
        ...
        assert_eq!(manifest.slots.len(),75);
        assert_eq!(model.cost_tree().n_slots(),75);
    }
}
