...
pub trait SweepCoords {
    ...
    /// Human-readable cache-coordinate names in `coords()` order. For direct
    /// scalar Inputs these are also physical field names. Manual projections
    /// over ragged or re-axis Inputs name the derived work coordinates instead.
    /// `kernel-query grid` uses these labels for presentation only.
