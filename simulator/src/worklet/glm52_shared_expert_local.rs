//! GLM-5.2 local shared-expert MLP worklet.
//!
//! Pinned serving uses one shared expert of width 2048: fused gate/up
//! projection → SiLU-and-multiply → down projection. This self-completing local
//! compute section intentionally excludes normalization, routing, routed-expert
//! work, communication, weighting, and the final routed/shared addition. L4
//! owns that explicit finalization boundary.
...
#[cfg(test)]
const SOURCE_ORDER: [&str; 3] = ["gate_up_proj", "silu_and_mul", "down_proj"];
...
#[derive(Clone, Debug)]
pub struct Glm52SharedExpertLocalWorkletResolved {
    pub raw_cfg: Glm52SharedExpertLocalWorkletConfig,
...
pub struct Glm52SharedExpertLocalWorkletInput {
    pub batch_tokens: u32,
}

pub struct Glm52SharedExpertLocalWorklet {
    pub name: String,
    pub gate_up_proj: Op<SingleGemmKernel>,
    pub silu_and_mul: Op<ElementwiseKernel>,
    pub down_proj: Op<SingleGemmKernel>,
    resolved: Glm52SharedExpertLocalWorkletResolved,
}
...
impl Glm52SharedExpertLocalWorklet {
    ...
    pub fn resolve_config(
        ...
        validate_config(cfg).unwrap_or_else(|reason| {
            panic!("invalid Glm52SharedExpertLocalWorkletConfig: {reason}")
        });
...
fn validate_config(cfg: &Glm52SharedExpertLocalWorkletConfig) -> Result<(), String> {
...
fn worklet_label(name: &str, cfg: &Glm52SharedExpertLocalWorkletConfig) -> String {
    let shared_width = checked_product(
...
mod tests {
    ...
    fn cfg() -> Glm52SharedExpertLocalWorkletConfig {
    ...
    fn source_order_is_exact_local_and_excludes_other_sections() {
        assert_eq!(SOURCE_ORDER, ["gate_up_proj", "silu_and_mul", "down_proj"]);
    ...
    fn resolve_bakes_exact_one_shared_expert_shapes_bytes_and_backends() {
        let r = Glm52SharedExpertLocalWorklet::resolve_config(&cfg());
    ...
    fn input_and_zero_case_feed_all_three_leaves_faithfully() {
        ...
        assert_eq!(
            Glm52SharedExpertLocalWorkletInput::default().batch_tokens,
            0
        );
    }
