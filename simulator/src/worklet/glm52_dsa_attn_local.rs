//! GLM-5.2 local DSA attention worklet.
//!
...
#[cfg(test)]
const SOURCE_ORDER_WITH_INDEXER: [&str; 11] = [
    "input_add_rms_norm",
    "fused_qkv_a_proj",
    "q_a_rms_norm",
    "q_b_proj",
...
#[cfg(test)]
const SOURCE_ORDER_INDEX_SHARE: [&str; 10] = [
    "input_add_rms_norm",
    "fused_qkv_a_proj",
    "q_a_rms_norm",
    "q_b_proj",
...
#[derive(Clone, Debug)]
pub struct Glm52DsaAttnLocalWorkletConfig {
...
pub struct Glm52DsaAttnLocalWorklet {
    ...
    resolved: Glm52DsaAttnLocalWorkletResolved,
}
...
impl Glm52DsaAttnLocalWorklet {
    ...
    pub fn resolve_config(
        ...
        validate_config(cfg)
            .unwrap_or_else(|reason| panic!("invalid Glm52DsaAttnLocalWorkletConfig: {reason}"));
        ...
        Glm52DsaAttnLocalWorkletResolved {
            ...
            sparse_mla: DsaSparseMlaAttentionConfig {
                ...
                dtype: cfg.base_dtype,
                attention_q_dtype: cfg.base_dtype,
                attention_cache_dtype: cfg.base_dtype,
                attention_output_dtype: cfg.base_dtype,
                index_dtype: cfg.index_dtype.clone(),
    ...
    pub fn eval(&self, input: &Glm52DsaAttnLocalWorkletInput, ev: &mut Evaluator) {
        let normalized = normalize_input(
            input,
            self.resolved.raw_cfg.decode_next_n,
            self.resolved.raw_cfg.max_model_len.get(),
        )
        .unwrap_or_else(|reason| panic!("invalid Glm52DsaAttnLocalWorkletInput: {reason}"));
...
fn validate_config(cfg: &Glm52DsaAttnLocalWorkletConfig) -> Result<(), String> {
...
fn normalize_input(
    input: &Glm52DsaAttnLocalWorkletInput,
    decode_next_n: u32,
    max_model_len: u32,
...
fn worklet_label(name: &str, cfg: &Glm52DsaAttnLocalWorkletConfig) -> String {
    format!(
        "{name} (Glm52DsaAttnLocalWorklet) [local (1 GPU); indexer={}; heads={}; next_n={}]",
...
mod tests {
    ...
    fn cfg(include_indexer: bool, decode_next_n: u32) -> Glm52DsaAttnLocalWorkletConfig {
    ...
    fn source_order_is_exact_and_excludes_other_model_sections() {
        ...
        assert_eq!(SOURCE_ORDER_INDEX_SHARE.len(), 10);
    ...
    fn resolve_bakes_every_exact_projection_bmm_and_sparse_shape() {
        let r = Glm52DsaAttnLocalWorklet::resolve_config(&cfg(true, 1));
    ...
    fn indexer_selection_is_structural_and_preserves_h32_h64_distinction() {
        let full = Glm52DsaAttnLocalWorklet::resolve_config(&cfg(true, 1));
        ...
        let shared = Glm52DsaAttnLocalWorklet::resolve_config(&cfg(false, 1));
    ...
    fn zero_case_and_num_new_token_equality_are_enforced() {
        ...
        let mismatched = Glm52DsaAttnLocalWorkletInput {
            num_new_tokens: 7,
    ...
    fn malformed_prefill_decode_and_row_overflow_fail_clearly() {
        ...
        let sum_overflow = Glm52DsaAttnLocalWorkletInput {
            num_new_tokens: 0,
    ...
    fn unsupported_glm_identity_fails_during_pure_resolution() {
        for mutate in [
            |cfg: &mut Glm52DsaAttnLocalWorkletConfig| cfg.num_attention_heads = 32.into(),
            |cfg: &mut Glm52DsaAttnLocalWorkletConfig| cfg.model_num_index_heads = 64.into(),
            |cfg: &mut Glm52DsaAttnLocalWorkletConfig| cfg.profile_num_index_heads = 32.into(),
            |cfg: &mut Glm52DsaAttnLocalWorkletConfig| cfg.base_dtype = DType::Fp32,
            |cfg: &mut Glm52DsaAttnLocalWorkletConfig| cfg.index_cache_dtype = DType::Bf16,
            |cfg: &mut Glm52DsaAttnLocalWorkletConfig| cfg.selected_k = 1024,
            |cfg: &mut Glm52DsaAttnLocalWorkletConfig| {
