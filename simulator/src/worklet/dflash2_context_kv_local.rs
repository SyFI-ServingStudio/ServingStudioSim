...
impl Dflash2ContextKvLocalWorklet {
    ...
    pub fn resolve_config(
        ...
        validate_config(cfg)
            .unwrap_or_else(|reason| panic!("invalid Dflash2ContextKvLocalWorkletConfig: {reason}"));
        ...
        let fused_kv_n = checked_product(
            "fused_kv_proj.n",
            &[cfg.num_draft_layers, 2, kv_width],
        )
    ...
    pub fn eval(&self, input: &Dflash2ContextKvLocalWorkletInput, ev: &mut Evaluator) {
        ...
        eval_atomic_or_zero(&self.hidden_norm, RmsNormKernelInput { m: tokens }, zero, ev);
        eval_atomic_or_zero(
            &self.fused_kv_proj,
            SingleGemmKernelInput { m: tokens },
            zero,
...
mod tests {
    ...
    #[test]
    fn source_order_is_exact_local_and_excludes_the_per_layer_append() {
        assert_eq!(
            SOURCE_ORDER,
            [
                "aux_hidden_fc",
    ...
    #[test]
    fn the_aux_projection_reads_every_target_layer_it_is_given() {
        let resolved = Dflash2ContextKvLocalWorklet::resolve_config(&cfg());
        // `fc` consumes the six concatenated target hidden states.
        assert_eq!(resolved.aux_hidden_fc.k.get(), NUM_AUX_LAYERS * HIDDEN_DIM);
        assert_eq!(resolved.aux_hidden_fc.n.get(), HIDDEN_DIM);
    }
