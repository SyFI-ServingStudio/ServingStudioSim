...
use crate::op::ssm::{
    GdnDecodeOp, GdnDecodeOpConfig, GdnDecodeOpInput, GdnPrefillOp, GdnPrefillOpConfig,
    GdnPrefillOpInput,
};
use crate::op::Op;
...
use crate::timing::kernels::{
    ...
    GdnGatedRmsNormKernelConfig, GdnGatedRmsNormKernelInput, GdnPrefillPostConvKernelConfig,
    GdnRecurrentDecodeKernelConfig, ResidualRmsNormKernel, ResidualRmsNormKernelConfig,
    ResidualRmsNormKernelInput, SingleGemmKernel, SingleGemmKernelConfig, SingleGemmKernelInput,
};
use crate::timing::{
...
impl Qwen36GdnLocalWorklet {
    ...
    pub fn build(
        ...
        let qkvz =
            SingleFp8GemmWithQuantOp::build(format!("{name}.qkvz"), resolved.qkvz.clone(), bridge)?;
        let ba = build_atomic(
            &name,
            "ba",
            resolved.ba.clone(),
        ...
        let split_b = build_atomic(
            &name,
            "split_b",
            resolved.split_b.clone(),
            ElementwiseKernel::build,
            bridge,
        )?;
        let split_a = build_atomic(
            &name,
            "split_a",
            resolved.split_a.clone(),
            ElementwiseKernel::build,
            bridge,
        )?;
        let core_output_zero = build_atomic(
            &name,
            "core_output_zero",
            resolved.core_output_zero.clone(),
            ElementwiseKernel::build,
            bridge,
        )?;
        let state_gather = build_atomic(
            &name,
            "state_gather",
            resolved.state_gather.clone(),
            ElementwiseKernel::build,
            bridge,
        )?;
        let state_zero = build_atomic(
            &name,
            "state_zero",
            resolved.state_zero.clone(),
            ElementwiseKernel::build,
            bridge,
        )?;
        let prefill =
            GdnPrefillOp::build(format!("{name}.prefill"), resolved.prefill.clone(), bridge)?;
        let state_scatter = build_atomic(
            &name,
            "state_scatter",
            resolved.state_scatter.clone(),
            ElementwiseKernel::build,
            bridge,
        )?;
        let decode = GdnDecodeOp::build(format!("{name}.decode"), resolved.decode.clone(), bridge)?;
        let core_output_copy = build_atomic(
            &name,
            "core_output_copy",
            resolved.core_output_copy.clone(),
            ElementwiseKernel::build,
            bridge,
        )?;
        let gated_norm = build_atomic(
            &name,
            "gated_norm",
            resolved.gated_norm.clone(),
    ...
    pub fn compile(&self, builder: &mut CostTreeBuilder) -> CostNode {
        ...
        CostNode::Labeled {
            ...
            child: Box::new(CostNode::Sum(vec![
                ...
                self.qkvz.compile(builder),
                self.ba.compile(builder),
                self.split_b.compile(builder),
                self.split_a.compile(builder),
                self.core_output_zero.compile(builder),
                self.state_gather.compile(builder),
                self.state_zero.compile(builder),
                self.prefill.compile(builder),
                self.state_scatter.compile(builder),
                self.decode.compile(builder),
                self.core_output_copy.compile(builder),
    ...
    pub fn eval(&self, input: &Qwen36GdnLocalWorkletInput, ev: &mut Evaluator) {
        let work = derive_work(input)
            .unwrap_or_else(|reason| panic!("invalid Qwen36GdnLocalWorkletInput: {reason}"));
        ...
        eval_atomic_or_zero(
            ...
            work.post_attention_add_rms_norm,
            false,
            ev,
        );
    }
...
fn derive_work(input: &Qwen36GdnLocalWorkletInput) -> Result<WorkInputs, String> {
    ...
    let total = prefill_tokens
        ...
        .ok_or("total-token sum overflow")?;
    if total == 0 {
        return Err("total token count must be positive".into());
    }
    let total = u32::try_from(total).map_err(|_| "total token count exceeds u32")?;
    let num_prefill = u32::try_from(input.prefill_sequence_lengths.len())
        .map_err(|_| "prefill count exceeds u32")?;
    let num_fresh = u32::try_from(
        input
    ...
    .map_err(|_| "fresh-prefill count exceeds u32")?;
    ...
    let norm_rows = total
        ...
        .ok_or("gated norm row count overflow")?;
    let mixed = prefill_tokens > 0 && input.decode_batch_size > 0;
    let copy_tokens = if mixed {
        total
            .checked_mul(2)
            .ok_or("mixed core-copy count overflow")?
    } else {
        total
    };
    Ok(WorkInputs {
        ...
        state_gather: ElementwiseKernelInput {
            num_tokens: prefill_state_units,
        },
        state_zero: ElementwiseKernelInput {
            num_tokens: fresh_state_units,
        },
        prefill: GdnPrefillOpInput {
            sequence_lengths: input.prefill_sequence_lengths.clone(),
        },
        state_scatter: ElementwiseKernelInput {
            num_tokens: prefill_state_units,
        },
...
fn validate_config(cfg: &Qwen36GdnLocalWorkletConfig) -> Result<(), String> {
    for (name, actual, expected) in [
        ...
        ("num_key_heads", cfg.num_key_heads.get(), NUM_KEY_HEADS),
        (
            "num_value_heads",
            cfg.num_value_heads.get(),
            NUM_VALUE_HEADS,
        ),
        ("key_head_dim", cfg.key_head_dim.get(), KEY_HEAD_DIM),
        ("value_head_dim", cfg.value_head_dim.get(), VALUE_HEAD_DIM),
        (
            "conv_kernel_size",
            cfg.conv_kernel_size.get(),
            CONV_KERNEL_SIZE,
        ),
    ] {
        if actual != expected {
    ...
    if cfg.conv_state_dtype != DType::Bf16 {
        return Err("conv_state_dtype must be BF16".into());
    }
    if cfg.ssm_state_dtype != DType::Fp32 {
        return Err("ssm_state_dtype must be FP32".into());
    }
    Ok(())
}
...
mod tests {
    ...
    fn cfg() -> Qwen36GdnLocalWorkletConfig {
        Qwen36GdnLocalWorkletConfig {
            hidden: HIDDEN.into(),
            num_key_heads: NUM_KEY_HEADS.into(),
            num_value_heads: NUM_VALUE_HEADS.into(),
            key_head_dim: KEY_HEAD_DIM.into(),
            value_head_dim: VALUE_HEAD_DIM.into(),
            conv_kernel_size: CONV_KERNEL_SIZE.into(),
            ...
            causal_conv_prefill_backends: vec!["vllm_triton"],
            prefill_post_conv_backends: vec!["vllm_triton"],
            chunk_delta_rule_backends: vec!["flashinfer"],
            causal_conv_decode_backends: vec!["vllm_triton"],
            recurrent_decode_backends: vec!["vllm_triton"],
            gated_rms_norm_backends: vec!["vllm_triton"],
        }
    }
    ...
    fn enumerate_op() -> Qwen36GdnLocalWorklet {
        ...
        Qwen36GdnLocalWorklet::build(
            "model.gdn".into(),
            Qwen36GdnLocalWorklet::resolve_config(&cfg()),
            &bridge,
        )
        .unwrap()
    }
    ...
    fn resolution_freezes_projection_shapes_dtypes_and_backends() {
        ...
        assert_eq!(
            (
                r.qkvz.quant.hidden_size.get(),
                r.qkvz.gemm.k.get(),
                r.qkvz.gemm.n.get()
            ),
            (2048, 2048, 12288)
        );
        assert_eq!(r.qkvz.quant.group_size, 128);
        assert_eq!(r.qkvz.quant.input_dtype, DType::Bf16);
        assert_eq!(r.qkvz.quant.scale_format, SCALE_FORMAT);
        assert_eq!(r.qkvz.gemm.dtype, DType::Fp8E4m3);
        assert_eq!(r.qkvz.gemm.backends, ["deepgemm"]);
        assert_eq!(
            (r.ba.k.get(), r.ba.n.get(), r.ba.dtype),
            (2048, 64, DType::Bf16)
        );
        assert_eq!(r.ba.backends, ["torch_linear"]);
        assert_eq!(
            (
                r.out_proj.quant.hidden_size.get(),
                r.out_proj.gemm.k.get(),
                r.out_proj.gemm.n.get()
            ),
            (4096, 4096, 2048)
        );
        ...
        assert_eq!(
            (
                r.input_add_rms_norm.hidden.get(),
                r.post_attention_add_rms_norm.hidden.get()
            ),
            (2048, 2048)
        );
    }
    ...
    fn resolution_freezes_bytes_and_state_dtypes() {
        let r = Qwen36GdnLocalWorklet::resolve_config(&cfg());
        let rates = |c: &ElementwiseKernelConfig| {
            (
                c.input_bytes_per_token.get(),
                c.output_bytes_per_token.get(),
            )
        };
        assert_eq!(rates(&r.split_b), (64, 64));
        assert_eq!(rates(&r.split_a), (64, 64));
        assert_eq!(rates(&r.core_output_zero), (0, 8192));
        assert_eq!(
            STATE_TRANSFER_UNIT_BYTES * STATE_TRANSFER_UNITS_PER_SEQUENCE,
            2_097_152
        );
        assert_eq!(r.recurrent_state_bytes_per_sequence, 2_097_152);
        assert_eq!(rates(&r.state_gather), (4096, 4096));
        assert_eq!(rates(&r.state_zero), (0, 4096));
        assert_eq!(rates(&r.state_scatter), (4096, 4096));
        assert_eq!(rates(&r.core_output_copy), (8192, 8192));
        assert_eq!(r.prefill.causal_conv.state_dtype, DType::Bf16);
        assert_eq!(r.decode.causal_conv.state_dtype, DType::Bf16);
        assert_eq!(r.prefill.chunk_delta_rule.dtype, DType::Bf16);
        assert_eq!(r.prefill.chunk_delta_rule.backends, ["flashinfer"]);
        assert_eq!(r.prefill.chunk_delta_rule.gpu_name, "NVIDIA H200");
        assert_eq!(r.decode.recurrent.state_dtype, DType::Fp32);
        assert_eq!(r.gated_norm.hidden.get(), 128);
    }

    #[test]
    fn invalid_identity_and_state_relationships_are_rejected() {
        let mut bad = cfg();
        bad.hidden = 4096.into();
        assert!(std::panic::catch_unwind(|| Qwen36GdnLocalWorklet::resolve_config(&bad)).is_err());
        let mut bad = cfg();
        bad.conv_state_dtype = DType::Fp32;
        assert!(std::panic::catch_unwind(|| Qwen36GdnLocalWorklet::resolve_config(&bad)).is_err());
        let mut bad = cfg();
        bad.ssm_state_dtype = DType::Bf16;
        assert!(std::panic::catch_unwind(|| Qwen36GdnLocalWorklet::resolve_config(&bad)).is_err());
    }

    #[test]
    fn runtime_geometry_covers_decode_prefill_mixed_and_ragged() {
        let decode = derive_work(&Qwen36GdnLocalWorkletInput {
            decode_batch_size: 7,
            ..Default::default()
        })
        .unwrap();
        assert_eq!(
            (
                decode.qkvz.num_tokens,
                decode.num_prefill,
                decode.num_fresh,
                decode.core_output_copy.num_tokens
            ),
            (7, 0, 0, 7)
        );
        assert_eq!(decode.gated_norm.m, 224);
        let prefill = derive_work(&Qwen36GdnLocalWorkletInput {
            prefill_sequence_lengths: vec![128],
            prefill_has_initial_state: vec![true],
            decode_batch_size: 0,
        })
        .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            (
                prefill.state_gather.num_tokens,
                prefill.state_zero.num_tokens,
                prefill.state_scatter.num_tokens
            ),
            (512, 0, 512)
        );
        let mixed = derive_work(&Qwen36GdnLocalWorkletInput {
            prefill_sequence_lengths: vec![3, 65, 2],
            prefill_has_initial_state: vec![false, true, false],
            decode_batch_size: 5,
        })
        .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            (
                mixed.qkvz.num_tokens,
                mixed.core_output_copy.num_tokens,
                mixed.gated_norm.m
            ),
            (75, 150, 2400)
        );
        assert_eq!(
            (
                mixed.state_gather.num_tokens,
                mixed.state_zero.num_tokens,
                mixed.state_scatter.num_tokens
            ),
            (1536, 1024, 1536)
        );
    }
    ...
    fn state_transfer_units_preserve_exact_semantic_bytes_and_check_overflow() {
        ...
        for sequence_count in [0, 1, 3] {
            ...
            assert_eq!(
                u64::from(units) * u64::from(STATE_TRANSFER_UNIT_BYTES),
                semantic_bytes
            );
            ...
            assert_eq!(
                (
                    u64::from(units) * gather_input,
                    u64::from(units) * gather_output
                ),
                (semantic_bytes, semantic_bytes)
            );
            assert_eq!(
                (
                    u64::from(units) * zero_input,
                    u64::from(units) * zero_output
                ),
                (0, semantic_bytes)
            );
            assert_eq!(
                (
                    u64::from(units) * scatter_input,
                    u64::from(units) * scatter_output
                ),
                (semantic_bytes, semantic_bytes)
            );
        }
    ...
    fn runtime_validation_rejects_malformed_and_overflowing_inputs() {
        ...
        assert!(derive_work(&Qwen36GdnLocalWorkletInput {
            prefill_sequence_lengths: vec![1],
            prefill_has_initial_state: vec![],
            decode_batch_size: 0
        })
        .is_err());
        assert!(derive_work(&Qwen36GdnLocalWorkletInput {
            prefill_sequence_lengths: vec![0],
            prefill_has_initial_state: vec![false],
            decode_batch_size: 1
        })
        .is_err());
        assert!(derive_work(&Qwen36GdnLocalWorkletInput {
            prefill_sequence_lengths: vec![u32::MAX],
            prefill_has_initial_state: vec![true],
            decode_batch_size: 1
        })
        .is_err());
        assert!(derive_work(&Qwen36GdnLocalWorkletInput {
            decode_batch_size: u32::MAX,
            ..Default::default()
        })
        .is_err());
    }
    ...
    fn compile_has_exact_fifteen_children_and_twenty_flattened_leaves() {
        ...
        assert_eq!(
            tree.slots
                .iter()
                .map(|s| s.name.strip_prefix("model.gdn.").unwrap())
                .collect::<Vec<_>>(),
            expected
        );
        assert_eq!(tree.slots.first().unwrap().kind, "residual_rms_norm");
        assert_eq!(tree.slots.last().unwrap().kind, "residual_rms_norm");
        assert!(!tree.slots.iter().any(|slot| matches!(
            slot.kind.as_str(),
            "all_reduce" | "all_to_all" | "send_recv"
        )));
        if let CostNode::Labeled { child, .. } = tree.root {
            if let CostNode::Sum(children) = *child {
    ...
    fn compiled_configs_preserve_all_backend_roles() {
        let op = enumerate_op();
        assert_eq!(op.input_add_rms_norm.kernel.config.backends, ["vllm_cuda"]);
        assert_eq!(op.ba.kernel.config.backends, ["torch_linear"]);
        assert_eq!(op.split_b.kernel.config.backends, ["triton"]);
        assert_eq!(op.prefill.causal_conv.config.backends, ["vllm_triton"]);
        assert_eq!(op.prefill.chunk_delta_rule.config.dtype, DType::Bf16);
        assert_eq!(op.prefill.chunk_delta_rule.config.backends, ["flashinfer"]);
        assert_eq!(op.prefill.chunk_delta_rule.config.gpu_name, "NVIDIA H200");
        assert_eq!(op.decode.recurrent.config.backends, ["vllm_triton"]);
        assert_eq!(op.gated_norm.kernel.config.backends, ["vllm_triton"]);
    }
