//! `worklet` (L3) — model-module-level op compositions with a `lookup` cost
//! function. Each worklet is a hand-written struct of L2 op slots exposing the
//! eight-piece interface (`*Config` / `*Resolved` / `*Input` / `Self` /
//! `resolve_config` / `init_ops` / `dry_run_init_ops` / `lookup` + `Describe`).
