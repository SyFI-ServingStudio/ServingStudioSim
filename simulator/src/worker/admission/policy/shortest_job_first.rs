...
impl PendingOrderPolicy for ShortestJobFirst {
    ...
    fn contains(&self, request: RequestId) -> bool {
        self.queue
            .iter()
            .any(|candidate| candidate.0.request == request)
    }
...
mod tests {
    ...
    fn selects_the_smallest_total_job() {
        ...
        assert_eq!(policy.peek().unwrap().request, RequestId(1));
        assert_eq!(policy.pop(&mut ()).unwrap().request, RequestId(1));
    ...
    fn remove_updates_membership_and_queued_kv() {
        ...
        assert_eq!(policy.remove(RequestId(0)).unwrap().request, RequestId(0));
